...
def test_tiller():
    ...
    _cluster, cluster_client = cluster_and_client(cluster.id, admin_client)
...
def test_tiller1():
    ...
    app = proj_client.reload(app)
    # test for tiller to be stuck on bad installs
    wait_for_condition(proj_client, app, check_condition('Installed', 'True'))
    app = wait_for_condition(proj_client, app, lambda x: x.state == 'active',
